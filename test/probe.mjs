...
await renderUntilStable(element, 'settle')
await wait(20)
for (let i = 0; i < 6; i += 1) {
  const t = process.hrtime.bigint()
  const tree = renderRoot(element, 'settle')
  const ms = Number(process.hrtime.bigint() - t) / 1e6
  console.log('pass ' + i + ': ' + ms.toFixed(3) + 'ms  nodes=' + collect(tree).length + ' rows=' + byClass(tree, 'gitops-crow').length)
  await wait(2)
}
